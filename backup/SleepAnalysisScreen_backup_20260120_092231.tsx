...
export default function SleepAnalysisScreen({ hideHeader = false, isSubcomponent = false }: any) {
  ...
  return (
      ...
      >
        {isLoading && sleepHistory.length === 0 ? (
          ...
          <>
                  ...
                  {recordings.length > 0 ? recordings.map((rec, i) => (
                    <View key={i} style={styles(theme, isDark).recordingItem}>
                      <View style={styles(theme, isDark).recordingIconObj}>
                        <Mic size={16} color={rec.type === 'snoring' ? '#F59E0B' : '#8B5CF6'} />
                      </View>
                      <View style={{ flex: 1 }}>
                        <Text style={styles(theme, isDark).recordingTitle}>
                          {rec.type === 'snoring' ? 'Snoring Detected' : rec.type === 'sleep_talk' ? 'Sleep Talking' : 'Noise Event'}
                        </Text>
                        ...
                        {playingAudio === `rec_${i}` && playbackStatus && (
                          <View style={{ height: 2, backgroundColor: 'rgba(255,255,255,0.1)', marginTop: 8, borderRadius: 1, overflow: 'hidden', width: '90%' }}>
            ...
            <Text style={[styles(theme, isDark).stackStatus, { color: '#10B981' }]}>{latestSession ? 'Good' : '—'}</Text>
          </View>
          <View style={styles(theme, isDark).consistencyInfo}>
            <View style={styles(theme, isDark).consistRing}>
              <CircularProgress size={74} strokeWidth={7} score={latestSession?.sleepScore || 0} showText={true} />
            </View>
            <View style={styles(theme, isDark).consistencyData}>
              <View style={styles(theme, isDark).consistRow}>
                <Clock size={16} color="#A8B5C7" />
                <View style={{ flex: 1 }}>
                  <Text style={styles(theme, isDark).consistLabel}>Bedtime</Text>
                  <Text style={styles(theme, isDark).consistText}>
                    {latestSession ? new Date(latestSession.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '—'}
                  </Text>
                </View>
              </View>
              <View style={styles(theme, isDark).consistRow}>
                <Clock size={16} color="#A8B5C7" />
                <View style={{ flex: 1 }}>
                  <Text style={styles(theme, isDark).consistLabel}>Wake</Text>
                  <Text style={styles(theme, isDark).consistText}>
                    {latestSession ? new Date(latestSession.endTime || Date.now()).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '—'}
                  </Text>
                </View>
              </View>
            </View>
    ...
    {/* NEW SECTION: Sleep Regularity */ }
    < GlassModule title = "Sleep Regularity" icon = { TrendingUp } pro = {!isPremium
} theme = { theme } isDark = { isDark } onUnlock = { handleUnlock } >
  ...
  {/* NEW SECTION: Environmental Analysis */ }
